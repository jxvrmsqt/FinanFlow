...
const App: React.FC = () => {
  ...
  const calculateSmartPriorityScore = (debt: Debt) => {
    let score = 0;
    if (debt.isAgreement) score += 1000; // Acordos são prioridade máxima
    score += debt.interestRate * 50; // Juros altos aumentam muito a prioridade
    score += (debt.totalBalance / 1000) * 5; // Saldo maior tem peso levemente maior
  ...
  const sortedDebts = useMemo(() => {
    return [...data.debts]
      ...
      .sort((a, b) => calculateSmartPriorityScore(b) - calculateSmartPriorityScore(a));
  }, [data.debts]);
  ...
  return (
    <div className="min-h-screen flex flex-col md:flex-row font-sans text-slate-800 bg-[#f8f9fe]">
      ...
      <main className="flex-1 md:ml-[280px] p-6 md:p-12 overflow-x-hidden">
        ...
        {activeTab === 'home' && (
          <div className="space-y-12">
            ...
            <section className="grid grid-cols-1 lg:grid-cols-2 gap-8">
              <div className="bg-white rounded-[2.5rem] p-10 shadow-sm border border-gray-100">
                <div className="flex items-center justify-between mb-8">
                  <h3 className="text-xl font-black flex items-center gap-3"><TrendingUp className="text-blue-500" /> Evolução do Saldo Devedor</h3>
                </div>
                  ...
                  <ResponsiveContainer width="100%" height="100%">
                    <AreaChart data={historyChartData}>
                      ...
                      <Tooltip 
                        ...
                        formatter={(value: number) => [`R$ ${value.toLocaleString('pt-BR')}`, 'Saldo Devedor']}
                      />
              ...
              <div className="bg-white rounded-[2.5rem] p-10 shadow-sm border border-gray-100">
                <div className="flex items-center justify-between mb-8">
                  <h3 className="text-xl font-black flex items-center gap-3"><Scale className="text-indigo-500" /> Distribuição de Gastos</h3>
                </div>
            ...
            <section className="bg-white rounded-[2.5rem] p-10 shadow-sm border border-gray-100">
              <div className="flex items-center justify-between mb-8">
                <h3 className="text-xl font-black flex items-center gap-3"><ShieldAlert className="text-red-500" /> Prioridade de Pagamento (Snowball Method)</h3>
                <div className="bg-red-50 text-red-600 px-4 py-2 rounded-xl text-xs font-bold uppercase tracking-widest">Ação Necessária</div>
              </div>
              <div className="space-y-4">
                {sortedDebts.slice(0, 3).map((debt, idx) => (
                  <div key={debt.id} className="flex items-center justify-between p-6 bg-gray-50 rounded-2xl border border-gray-100 hover:border-blue-200 transition-all">
                    <div className="flex items-center gap-6">
                      <div className={`w-12 h-12 rounded-xl flex items-center justify-center font-black text-white ${idx === 0 ? 'bg-red-500 shadow-red-100' : 'bg-orange-400 shadow-orange-100'} shadow-lg`}>
                        {idx + 1}
                      </div>
                      <div>
                        <h4 className="font-black text-lg">{debt.name}</h4>
                        <div className="flex gap-4 mt-1">
                          <span className="text-xs font-bold text-gray-400 flex items-center gap-1"><Zap size={12}/> Juros: {debt.interestRate}%</span>
                          <span className="text-xs font-bold text-gray-400 flex items-center gap-1"><DollarSign size={12}/> Saldo: R$ {debt.totalBalance.toLocaleString('pt-BR')}</span>
                        </div>
                      </div>
                    </div>
                    <div className="text-right">
                      <p className="text-xs font-bold text-gray-400 uppercase tracking-widest mb-1">Score de Prioridade</p>
                      <p className="text-xl font-black text-gray-800">{calculateSmartPriorityScore(debt)}</p>
                    </div>
                  </div>
                ))}
              </div>
        ...
        {activeTab === 'history' && (
          <div className="space-y-12">
            <h2 className="text-4xl font-black">Evolução Histórica</h2>
            <div className="bg-white rounded-[2.5rem] p-10 shadow-sm border border-gray-100">
                ...
                <ResponsiveContainer width="100%" height="100%">
                  <ComposedChart data={historyChartData}>
                    ...
                    <Tooltip 
                      ...
                      formatter={(value: number) => [`R$ ${value.toLocaleString('pt-BR')}`, 'Saldo Devedor']}
                    />
                    <Bar dataKey="balance" barSize={40} fill="#e2e8f0" radius={[10, 10, 0, 0]} />
                    <Line type="monotone" dataKey="balance" stroke="#3b82f6" strokeWidth={4} dot={{ r: 6, fill: '#3b82f6', strokeWidth: 3, stroke: '#fff' }} />
                  </ComposedChart>
                </ResponsiveContainer>
              </div>
            </div>
        ...
        {activeTab === 'advisor' && (
          <div className="space-y-12">
            <div className="flex items-center justify-between">
              <h2 className="text-4xl font-black">IA Advisor</h2>
              <button onClick={handleGetAdvice} className="bg-black text-white px-8 py-4 rounded-2xl font-bold flex items-center gap-3 shadow-xl active:scale-95 transition-all">
                {loadingAdvice ? <RefreshCw className="animate-spin" /> : <Sparkles />}
                Nova Análise
              </button>
            </div>
            ...
            {aiAdvice ? (
              <div className="bg-indigo-600 rounded-[3rem] p-12 text-white shadow-2xl relative overflow-hidden">
                <div className="absolute top-0 right-0 p-8 opacity-10 rotate-12"><BrainCircuit size={150} /></div>
                <div className="relative z-10">
                  <div className="flex items-center gap-4 mb-8">
                    <div className="w-12 h-12 bg-white/20 rounded-2xl flex items-center justify-center"><Zap /></div>
                    <h3 className="text-2xl font-black">Diretrizes Estratégicas</h3>
                  </div>
                  <div className="text-lg leading-relaxed whitespace-pre-wrap font-medium text-indigo-50">
                    {aiAdvice}
                  </div>
                </div>
              </div>
              ...
              <div className="bg-white rounded-[3rem] p-20 text-center border-4 border-dashed border-gray-100">
                <div className="w-20 h-20 bg-gray-50 rounded-full flex items-center justify-center mx-auto mb-6"><Sparkles className="text-gray-300" size={32} /></div>
                <h3 className="text-2xl font-black text-gray-400">Clique em "Nova Análise" para receber diretrizes da IA baseadas em seus dados atuais.</h3>
              </div>
            )}
          </div>
        )}
